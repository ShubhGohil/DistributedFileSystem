...
void downltar(int connection_socket, int count, char *token[]) {
    ...
    char *base_dir = malloc(strlen(getenv("HOME") + strlen("/S2")));
    struct stat st;
...
int main() {
    ...
    char ip_str[INET6_ADDRSTRLEN], ip_str2[INET6_ADDRSTRLEN];
    ...
    if (client_addr.ss_family == AF_INET) {
        ...
        inet_ntop(AF_INET, &s->sin_addr, ip_str2, sizeof(ip_str));
    } else { // AF_INET6
        struct sockaddr_in6 *s = (struct sockaddr_in6 *)&client_addr;
        port = ntohs(s->sin6_port);
        inet_ntop(AF_INET6, &s->sin6_addr, ip_str2, sizeof(ip_str));
    }
